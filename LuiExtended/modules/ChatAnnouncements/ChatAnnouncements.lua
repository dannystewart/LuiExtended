...
--> Condition: %s]],
    ...
    do
        ...
        local originalShowPlayerInteractMenu = PLAYER_TO_PLAYER.ShowPlayerInteractMenu
        --- @param self ZO_PlayerToPlayer
        --- @param isIgnored boolean
        function PLAYER_TO_PLAYER:ShowPlayerInteractMenu(isIgnored)
    ...
    local function CompleteGroupInvite(characterOrDisplayName, sentFromChat, displayInvitedMessage, isMenu)
        ...
        if isLeader and groupSize == SMALL_GROUP_SIZE_THRESHOLD then
            ...
            if isMenu then
                local link
                if ChatAnnouncements.SV.BracketOptionCharacter == 1 then
                    link = ZO_LinkHandler_CreateLinkWithoutBrackets(characterOrDisplayName, nil, DISPLAY_NAME_LINK_TYPE, characterOrDisplayName)
                else
                    link = ZO_LinkHandler_CreateLink(characterOrDisplayName, nil, DISPLAY_NAME_LINK_TYPE, characterOrDisplayName)
                end
